...
/** "My Classes" on the faculty member dashboard: upload a timetable, Astro organizes it. */
class FacultyScheduleController extends Controller
{
    ...
    {
        if (! $this->isFaculty() || $class->user_id !== Auth::id()) {
    ...
    /** The faculty member's own "today" (the browser sends it), trusted only within a day or two of the server's clock. */
    ...
    private function isFaculty(): bool
    {
        return Auth::check() && (Auth::user()->role ?? 'student') === 'faculty';
    }
}
