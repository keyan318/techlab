...
/**
 * Teacher-authored quizzes. Teachers create/delete them from their dashboard; students in the
...
class CrewQuizController extends Controller
{
    ...
    {
        abort_unless(Auth::check() && (int) Auth::user()->crew_id === (int) $quiz->crew_id, 403);
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        return redirect(route('teacher.crew').'#quizzes');
    }

    public function destroy(CrewQuiz $quiz): RedirectResponse
    {
        abort_unless($quiz->crew_id === $this->ownCrew()->id, 403);
        $quiz->delete();
