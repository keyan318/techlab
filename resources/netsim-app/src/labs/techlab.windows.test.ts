// The same solvability guard as techlab.test.ts, but every command a student types is the
// Windows one (TechLab labs run NetSim in Windows mode). toWin() maps each step 1:1, so the
// two files stay in lockstep: a new Linux step without a Windows mapping fails loudly.
...
import { prefixToMask } from '../commands/windows/util';
...
const adapter = (eth: string) => (eth === 'eth0' ? 'Ethernet' : `Ethernet ${Number(eth.slice(3)) + 1}`);

function toWin(line: string): string {
  let m: RegExpMatchArray | null;
  if ((m = line.match(/^ip addr add (\S+)\/(\d+) dev (eth\d+)$/)))
    return `netsh interface ip set address "${adapter(m[3])}" static ${m[1]} ${prefixToMask(Number(m[2]))}`;
  if ((m = line.match(/^ip addr del (\S+)\/\d+ dev (eth\d+)$/))) return `netsh interface ip delete address "${adapter(m[2])}" ${m[1]}`;
  if ((m = line.match(/^ip route add default via (\S+)$/))) return `route add 0.0.0.0 mask 0.0.0.0 ${m[1]}`;
  if ((m = line.match(/^ip route add (\S+)\/(\d+) via (\S+)$/))) return `route add ${m[1]} mask ${prefixToMask(Number(m[2]))} ${m[3]}`;
  if ((m = line.match(/^ip link set (eth\d+) up$/))) return `netsh interface set interface "${adapter(m[1])}" admin=enabled`;
  if (line === 'dhclient') return 'ipconfig /renew';
  if ((m = line.match(/^traceroute (\S+)$/))) return `tracert ${m[1]}`;
  if ((m = line.match(/^ping -c (\d+) (\S+)$/))) return `ping -n ${m[1]} ${m[2]}`;
  throw new Error(`no Windows translation for: ${line}`);
}

function sh(net: Network, d: IpDevice, line: string): void {
  let done = false;
  new WinShell(d).exec(toWin(line), { write: () => {} }, () => (done = true));
...
function out(net: Network, d: IpDevice, line: string): string {
  let text = '';
  let done = false;
  new WinShell(d).exec(toWin(line), { write: (t: string) => (text += t) }, () => (done = true));
  net.scheduler.advanceTo(net.scheduler.now + 60_000);
  expect(done, line).toBe(true);
  return text;
}
...
describe('[windows] every TechLab lab file is well formed', () => {
...
describe('[windows] m1-l1 First Contact', () => {
...
describe('[windows] m1-l2 Ship\'s Wiring', () => {
...
describe('[windows] m1-l3 Cables & Signals', () => {
...
describe('[windows] m1-l4 Switches, Routers & VLANs', () => {
...
describe('[windows] m1-l5 Servers & Virtualization', () => {
...
describe('[windows] m1-l6 Cloud Relay', () => {
...
describe('[windows] m2-l1 Climb the Layers', () => {
...
describe('[windows] m2-l2 Standard Ports', () => {
...
describe('[windows] m2-l3 Private Addresses', () => {
...
describe('[windows] m2-l4 Network or Host?', () => {
...
describe('[windows] m2-l5 Split the Block', () => {
...
describe('[windows] m2-l6 Static Routes', () => {
...
describe('[windows] m2-l7 Midterm Mission', () => {
...
describe('[windows] m3-l1 Static and Dynamic', () => {
  it('needs a static server, then a DHCP server before the crew can lease', () => {
    ...
    for (const n of ['astro', 'rivet', 'volt']) expect(out(net, dev(net, n), 'dhclient')).toContain('unable to contact your DHCP server');
    expect(allPass(grade(net, s2))).toBe(false);
...
describe('[windows] m3-l2 The DHCP Lease Process', () => {
  it('shows the four steps once the server is on, and hands different clients different addresses', () => {
    const f = load('m3-l2');
    const net = restoreNetwork(f);
    const [s1, s2] = steps(f);
    expect(out(net, dev(net, 'astro'), 'dhclient')).toContain('unable to contact your DHCP server');
    expect(allPass(grade(net, s1))).toBe(false);

    dev(net, 'ship-server').dhcpServer!.enabled = true;
    const text = out(net, dev(net, 'astro'), 'dhclient');
    for (const step of ['DISCOVER', 'OFFER', 'REQUEST', 'ACK']) expect(text).toContain(step);
    expect(allPass(grade(net, s1))).toBe(true);
...
describe('[windows] m3-l3 Centralized DHCP', () => {
  it('serves the comms deck, then shows broadcasts stopping at the router until gate serves the far deck', () => {
    ...
    expect(out(net, dev(net, 'volt'), 'dhclient')).toContain('unable to contact your DHCP server');
    expect(allPass(grade(net, s2))).toBe(false);
...
describe('[windows] m3-l4 DHCP Server Settings', () => {
  it('leaves the third client without an address until the pool grows, then needs the options', () => {
    ...
    expect(results[0]).toContain('-> ACK');
    expect(results[1]).toContain('-> ACK');
    expect(results[2]).toContain('unable to contact your DHCP server');
    expect(allPass(grade(net, s1))).toBe(false);
...
describe('[windows] m3-l5 DNS Overview', () => {
...
describe('[windows] m4-l1 Network Management', () => {
...
describe('[windows] m4-l2 Physical Issues', () => {
...
describe('[windows] m4-l3 Logical Issues', () => {
...
describe('[windows] m4-l4 Wireless Issues', () => {
  it('needs the AP uplink VLAN fixed, then a bigger DHCP pool for the second client', () => {
    ...
    expect(out(net, dev(net, 'laptop'), 'dhclient')).toContain('unable to contact your DHCP server');
    expect(allPass(grade(net, s1))).toBe(false);
    ...
    expect(out(net, dev(net, 'tablet'), 'dhclient')).toContain('unable to contact your DHCP server');
    expect(allPass(grade(net, s2))).toBe(false);
    dev(net, 'ship-server').dhcpServer!.rangeEnd = ip('192.168.1.150');
...
describe('[windows] m4-l5 Follow the Trail', () => {
  it('hides a return-path fault behind a forward-path fault', () => {
    ...
    expect(trace).not.toMatch(/ms\s+172\.16\.0\.10\b/);
...
describe('[windows] m4-l6 Final Mission: Call Earth', () => {
...
describe('[windows] ping with a CIDR address', () => {
  it('fails like Windows does, but tells the student what to change', () => {
    const net = restoreNetwork(load('m1-l1'));
    sh(net, dev(net, 'astro'), 'ip addr add 192.168.1.10/24 dev eth0');
    sh(net, dev(net, 'rivet'), 'ip addr add 192.168.1.20/24 dev eth0');

    const wrong = out(net, dev(net, 'astro'), 'ping -c 2 192.168.1.20/24');
    expect(wrong).toContain('could not find host');
    ...
    expect(right).toContain('Received = 2');
  });
});
