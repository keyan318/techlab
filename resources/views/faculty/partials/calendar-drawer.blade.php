{{--
  resources/views/faculty/partials/calendar-drawer.blade.php
  The faculty member's calendar: opened from the "Calendar" rail icon or the Classes page's "View Calendar" link.
  ...
  Included by components/shell/side-bar.blade.php for faculty only, so it exists on every faculty member page.
--}}
...
<script>
  document.addEventListener('alpine:init', () => {
    ...
    // Single source of truth for the faculty member's schedule. The Classes page seeds it via set(); other pages load it on first open.
    Alpine.store('calendar', {
      ...
      async load() {
        ...
        try {
          const r = await fetch(@json(route('faculty.schedule.index')), { credentials: 'same-origin', headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } });
      ...
      async saveClass() {
        ...
        try {
          const base = @json(route('faculty.schedule.class.store'));
      ...
      async removeClass(c) {
        ...
        try { const j = await this.api('DELETE', @json(route('faculty.schedule.class.store')) + '/' + c.id); this.set(j.classes); this.form = null; }
        catch (e) { this.error = e.message; }
      },
      async remove() {
        if (!confirm('Remove your whole schedule?')) return false;
        try { await this.api('DELETE', @json(route('faculty.schedule.destroy'))); this.set([]); return true; }
            ...
            <div class="cal-list">
              ...
              <div class="cal-empty" x-show="!selItems.length">
                <template x-if="!$store.calendar.classes.length">
                  <span>No classes yet. <a class="cal-link" href="{{ route('faculty.classes') }}">Upload your schedule</a> and Astro will lay it out here, or add one yourself.</span>
                </template>
                <template x-if="$store.calendar.classes.length">
