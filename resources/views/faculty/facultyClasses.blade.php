...
@section('content')
  {{-- CLASSES: faculty uploads a schedule (file or photo); Astro organizes it into upcoming classes.
    ...
    <section class="classes-card" aria-labelledby="classesTitle" x-data="scheduleCard()" x-cloak>
      ...
      {{-- Upcoming (next 4 slots, live "In 30 min" from the faculty member's own clock) --}}
...
<script>
  function scheduleCard() {
    ...
    return {
      ...
      async upload_(fd) {
        const res = await fetch(@json(route('faculty.schedule.store')), {
          method: 'POST', body: fd,
