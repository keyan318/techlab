...
@section('content')
  ...
  <section class="mods" id="modules">
        ...
        @foreach ($crew->modules as $m)
          <article class="mod" id="module-{{ $m->id }}">
            <div class="mod-top">
              ...
              <form method="POST" action="{{ route('teacher.modules.destroy', $m) }}" onsubmit="return confirm('Delete this module and all its files?')">
                @csrf @method('DELETE')
              ...
              <ul class="mfiles">
                @foreach ($m->materials as $f)
                  <li>
                    ...
                    <form method="POST" action="{{ route('teacher.materials.destroy', $f) }}" onsubmit="return confirm('Remove this file?')">
                      @csrf @method('DELETE')
            ...
            <form method="POST" action="{{ route('teacher.materials.store', $m) }}" enctype="multipart/form-data" class="mrow">
              @csrf
        ...
        <form method="POST" action="{{ route('teacher.modules.store') }}" class="mod mnew">
          @csrf
  ...
  <section class="mods" id="quizzes">
        ...
        @foreach ($crew->quizzes()->withCount(['questions', 'attempts'])->get() as $qz)
          ...
          <article class="mod">
            <div class="mod-top">
              ...
              <form method="POST" action="{{ route('teacher.quizzes.destroy', $qz) }}" onsubmit="return confirm('Delete this quiz and all student results for it?')">
                @csrf @method('DELETE')
        ...
        <form method="POST" action="{{ route('teacher.quizzes.store') }}" class="mod mnew" id="quizForm">
          @csrf
