...
class TeacherScheduleTest extends TestCase
{
    ...
    public function test_teacher_uploads_a_text_schedule_and_it_replaces_the_old_one(): void
    {
        $t = $this->teacher();
        ...
        $res = $this->actingAs($t)->post(route('teacher.schedule.store'), [
    ...
    public function test_an_image_schedule_is_transcribed_by_the_vision_model_then_structured(): void
    {
        ...
        $this->actingAs($t)->post(route('teacher.schedule.store'), ['file' => $this->drawn()], ['Accept' => 'application/json'])
    ...
    public function test_blurry_and_blank_images_are_rejected_locally_without_any_ai_call(): void
    {
        ...
        $this->actingAs($t)->post(route('teacher.schedule.store'), ['file' => $this->drawn('blur.png', blurry: true)], ['Accept' => 'application/json'])
            ->assertStatus(422)->assertJsonPath('error', fn ($e) => str_contains($e, 'sharper'));

        $this->actingAs($t)->post(route('teacher.schedule.store'), ['file' => UploadedFile::fake()->image('blank.png', 800, 600)], ['Accept' => 'application/json'])
    ...
    public function test_vision_saying_unreadable_stops_early_but_a_good_table_with_a_stray_word_still_works(): void
    {
        ...
        $this->actingAs($t)->post(route('teacher.schedule.store'), ['file' => $this->drawn()], ['Accept' => 'application/json'])->assertStatus(422);
        $this->actingAs($t)->post(route('teacher.schedule.store'), ['file' => $this->drawn()], ['Accept' => 'application/json'])
            ->assertOk()->assertJsonPath('classes.0.subject', 'Biology');
    }
    ...
    public function test_a_file_with_no_days_or_times_is_rejected_without_any_ai_call(): void
    {
        ...
        $this->actingAs($t)->post(route('teacher.schedule.store'), ['file' => UploadedFile::fake()->createWithContent('notes.txt', 'Remember to buy milk and call mum')], ['Accept' => 'application/json'])
    ...
    public function test_nothing_found_or_unreadable_file_keeps_the_existing_schedule(): void
    {
        ...
        $this->modelReturns(['classes' => []]);
        $this->actingAs($t)->post(route('teacher.schedule.store'), ['file' => UploadedFile::fake()->createWithContent('a.txt', 'Monday: sometime around nine')], ['Accept' => 'application/json'])
            ->assertStatus(422);

        $this->actingAs($t)->post(route('teacher.schedule.store'), ['file' => UploadedFile::fake()->create('a.exe', 5)], ['Accept' => 'application/json'])
            ->assertStatus(422);
    ...
    public function test_a_csv_schedule_never_touches_the_ai(): void
    {
        ...
        $this->actingAs($t)->post(route('teacher.schedule.store'), [
    ...
    public function test_live_mode_streams_stages_then_the_result(): void
    {
        ...
        $body = $this->actingAs($t)->post(route('teacher.schedule.store'), ['file' => $this->drawn()], ['Accept' => 'application/x-ndjson'])
    ...
    {
        $t = $this->teacher();
        $body = $this->actingAs($t)->post(route('teacher.schedule.store'), [
    ...
    public function test_index_returns_only_the_teachers_own_classes(): void
    {
        $t = $this->teacher();
        ...
        $this->actingAs($t)->getJson(route('teacher.schedule.index'))->assertOk()
            ->assertJsonCount(1, 'classes')->assertJsonPath('classes.0.subject', 'Mine');
        $this->actingAs(User::factory()->create(['role' => 'student']))->getJson(route('teacher.schedule.index'))->assertForbidden();
    }
